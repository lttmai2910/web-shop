...
import { ref } from 'vue';
    ...
    if (confirm("Bạn có chắc là muốn xóa nó chứ !!!")) {
        form.delete(route('productcategory.destroy', id), {
            onSuccess: () => {
    ...
    function create(){
        form.get(route('productcategory.create'));
    }
</script>
                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto">
                        <div class="relative flex flex-col md:flex-row w-auto font-lora">
                            ...
                            <div class="relative overflow-x-auto rounded-lg">
                                <table class="w-full text-sm text-left rtl:text-right">
                                    <thead class="text-xs text-white uppercase bg-emerald-700 dark:bg-emerald-700">
                                        <tr>
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-32">Mã danh mục</th>
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-48">Tên danh mục</th>
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-96">Mô tả</th>
                                            ...
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-48">Chức năng</th>
                                        </tr> 
                                    </thead>
                                    <tbody v-if='productcategory && productcategory.data.length > 0'>
                                        <tr v-for='(productcate, index) in productcategory.data' :key="index" class="bg-slate-50 border-b border-gray-300 hover:bg-gray-100 cursor-pointer">
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ productcate.id }}</td>
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ productcate.category_name }}</td>
                                            <td class="py-4 px-4 sm:px-3 text-center" style="max-width: 10cqi; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ productcate.description }}</td>
                                            <td class="py-4 px-4 sm:px-3 text-center">
                                                <img
                                                :src="'/image_productcategory/' + productcate.image"
                                                alt="Hình ảnh"
                                            ...
                                            <td class="px-4 py-1 sm:px-3 text-center">
                                                ...
                                                :class="{'text-green-500 bg-green-200': productcate.status === 1, 'text-red-500 bg-red-300': productcate.status === 0}">{{ productcate.status === 1 ? 'Đăng' : 'Ẩn' }}</p>
                                            </td>
                                            <td class="flex flex-row justify-center sm:py-16 py-6 px-4 sm:px-6 text-center">
                                                <a @click.prevent="destroy(productcate.id)" class="flex items-center justify-center w-8 h-8 cursor-pointer">
                                                ...
                                                <a :href="route('productcategory.edit', productcate.id)" class="flex items-center justify-center w-8 h-8">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 hover:text-emerald-600">
                            ...
                            <div class="flex justify-center">
                                <Pagination :links="productcategory.links" colorScheme="admin"/>
                            </div>                                
                        </div>
                    </div>
                </main>
