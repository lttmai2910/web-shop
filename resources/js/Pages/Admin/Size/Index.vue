<script setup>
    ...
    if (confirm("Bạn có chắc là muốn xóa nó chứ !!!")) {
        ...
        {
            onSuccess: () => {
                successMessage.value = 'Xóa size thành công !';
                        ...
                        <div class="relative overflow-x-auto rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right">
                                <thead class="text-xs text-white uppercase bg-emerald-700 dark:bg-emerald-700">
                                    <tr>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-96">Mã kích cỡ</th>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-96">Tên kích cỡ</th>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-96">Chức năng</th>
                                    </tr> 
                                </thead>
                                <tbody v-if='size && size.data.length > 0'>
                                    <tr v-for='(size, index) in size.data' :key="index" class="bg-slate-50 border-b border-gray-300 hover:bg-gray-100 cursor-pointer">
                                        <td class="py-4 px-4 sm:px-3 text-center">{{ size.id }}</td>
                                        <td class="py-4 px-4 sm:px-3 text-center">{{ size.size }}</td>
                                        <td class="flex flex-row justify-center sm:py-16 py-6 px-4 sm:px-6 text-center">
                                            <a @click="destroy(size.id)" class="flex items-center justify-center w-8 h-8">
                                            ...
                                            <a :href="route('size.edit', size.id)" class="flex items-center justify-center w-8 h-8">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 hover:text-emerald-600">
                            ...
                            <div class="flex justify-center">
                                <Pagination :links="size.links" colorScheme="admin"/>
                            </div>  
                        </div>
                    </div>
                </main>
