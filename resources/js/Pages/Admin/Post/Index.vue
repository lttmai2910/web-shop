<script setup>
    ...
    if (confirm("Bạn có chắc là muốn xóa nó chứ !!!")) {
        ...
        {
            onSuccess: () => {
                successMessage.value = 'Xóa tin tức thành công !';
                        ...
                        <div class="relative overflow-x-auto rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right">
                                <thead class="text-xs text-white uppercase bg-emerald-700 dark:bg-emerald-700">
                                    <tr>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-32">Mã tin tức</th>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-52">Tiêu đề</th>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-96">Nội dung</th>
                                        ...
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-32">Người đăng</th>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-24">Chức năng</th>
                                    </tr>
                                </thead>
                                <tbody v-if='post && post.data.length > 0'>
                                    <tr v-for='(ps, index) in post.data' :key="index" class="bg-slate-50 border-b border-gray-300 hover:bg-gray-100 cursor-pointer">
                                        <td class="py-4 px-4 sm:px-3 text-center">{{ ps.id }}</td>                                        
                                        <td class="py-4 px-4 sm:px-3 text-center">{{ ps.title }}</td>
                                        <td class="py-4 px-4 sm:px-3 text-center" style="max-width: 10cqi; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">
                                            {{ ps.body }}
                                        </td>
                                        <td class="py-4 px-4 sm:px-3 text-center">
                                            <img
                                            :src="'/image_post/' + ps.image"
                                            alt="Hình ảnh"
                                        ...
                                        <td class="px-4 py-1 sm:px-3 text-center">
                                            ...
                                            :class="{'text-green-500 bg-green-200': ps.status === 1, 'text-red-500 bg-red-300': ps.status === 0}">{{ ps.status === 1 ? 'Đăng' : 'Ẩn' }}</p>
                                        </td>
                                        <td class="py-4 px-4 sm:px-3 text-center">{{ ps.id_user }}</td>
                                        <td class="flex flex-row justify-center sm:py-16 py-6 px-4 sm:px-6 text-center">
                                            <a @click="destroy(ps.id)" class="flex items-center justify-center w-8 h-8">
                                            ...
                                            <a :href="route('post.edit', ps.id)" class="flex items-center justify-center w-8 h-8">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 hover:text-emerald-600">
                            ...
                            <div class="flex justify-center">
                                <Pagination :links="post.links" colorScheme="admin"/>
                            </div>                          
                        </div>
                    </div>
                </main>
