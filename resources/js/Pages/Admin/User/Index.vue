                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto">
                        <div class="relative flex flex-col md:flex-row w-auto font-lora">
                            ...
                            <div class="flex justify-between items-center">
                                ...
                                <!-- <form class="flex-grow ml-56">   
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="search" id="default-search" class="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-white focus:ring-slate-500 focus:border-red-500 dark:border-red-600 dark:placeholder-gray-400 dark:focus:ring-red-500 dark:focus:border-red-500" placeholder="Tìm kiếm..." required />
                            ...
                            <div class="relative overflow-x-auto rounded-lg">
                                <table class="w-full text-sm text-left rtl:text-right">
                                    <thead class="text-xs text-white uppercase bg-emerald-700 dark:bg-emerald-700">
                                        <tr>
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-24">Mã NV</th>
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-32">Tên NV</th>
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-24">Ngày sinh</th>
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-32">Địa chỉ</th>
                                            ...
                                            <th scope="col" class="px-6 py-3 sm:px-1 text-center w-24">Chức vụ</th>
                                    ...
                                    <tbody v-if='user && user.data.length > 0'>
                                        <tr v-for='(us, index) in user.data' :key="index" class="bg-slate-50 border-b border-gray-300 hover:bg-gray-100 cursor-pointer">
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ us.id }}</td>
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ us.name  }}</td>
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ us.birthday }}</td>
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ us.address }}</td>
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ us.gender === 1 ? 'Nam' : 'Nữ'}}</td>
                                            <td class="py-4 px-4 sm:px-3 text-center">
                                                <img
                                                :src="'/image_admin/' + us.image"
                                                alt="Hình ảnh"
                                                class="h-24 w-24 object-cover rounded-lg"
                                                >
                                            </td>
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ us.role === 1? 'Quản lý' : 'Nhân viên'}}</td>
                                            <td class="px-4 py-1 sm:px-3 text-center">
                                                <p 
                                                class="px-4 py-2 text-xs font-semibold rounded-full" 
                                                :class="{'text-green-500 bg-green-200': us.status === 1, 'text-red-500 bg-red-300': us.status === 0}">{{ us.status === 1 ? 'Hoạt động' : 'Xóa' }}</p>
                                            </td>
                                            <td class="py-4 px-4 sm:px-3 text-center">{{ us.email }}</td>                              
                                            <td class="flex flex-row justify-center sm:py-16 py-6 px-4 sm:px-6 text-center">
                                                <a @click="destroy(us.id)" class="flex items-center justify-center w-8 h-8">
                                                ...
                                                <a :href="route('user.edit', us.id)" class="flex items-center justify-center w-8 h-8">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 hover:text-emerald-600">
                            ...
                            <div class="flex justify-center">
                                <Pagination :links="user.links" colorScheme="admin"/>
                            </div>  
                        </div>
                    </div>
                </main>
