<script setup>
    ...
    if (confirm("Bạn có chắc là muốn xóa nó chứ !!!")) {
            form.delete(route('productgallery.destroy', id),
        {
            onSuccess: () => {
                successMessage.value = 'Xóa hình ảnh thành công !';
    ...
    function create(){
        form.get(route('productgallery.create'));
    }
                        ...
                        <div class="relative overflow-x-auto rounded-lg">
                            <table class="w-full text-sm text-left rtl:text-right">
                                <thead class="text-xs text-white uppercase bg-emerald-700 dark:bg-emerald-700">
                                    <tr>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-72">Mã hình ảnh sản phẩm</th>
                                        <th scope="col" class="px-6 py-3 sm:px-1 text-center w-72">Mã chi tiết sản phẩm</th>
                                ...
                                <tbody v-if='productgallery && productgallery.data.length > 0'>
                                    <tr v-for='(productga, index) in productgallery.data' :key="index" class="bg-slate-50 border-b border-gray-300 hover:bg-gray-100 cursor-pointer">
                                        <td class="py-4 px-4 sm:px-3 text-center">{{ productga.id }}</td>
                                        <td class="py-4 px-4 sm:px-3 text-center">{{ productga.id_product_item }}</td>
                                        <td class="py-4 px-4 sm:px-3 text-center">
                                            <img
                                            :src="'/image_productgall/' + productga.image"
                                            alt="Hình ảnh"
                                            class="h-24 w-24 object-cover rounded-lg"
                                            >
                                        </td>
                                        <td class="flex flex-row justify-center sm:py-16 py-6 px-4 sm:px-6 text-center">
                                            <a @click="destroy(productga.id)" class="flex items-center justify-center w-8 h-8">
                                            ...
                                            <a :href="route('productgallery.edit', productga.id)" class="flex items-center justify-center w-8 h-8">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 hover:text-emerald-600">
                            ...
                            <div class="flex justify-center mt-8">
                                <Pagination :links="productgallery.links" colorScheme="admin"/>
                            </div>
                        </div>
                    </div>
                </main>
